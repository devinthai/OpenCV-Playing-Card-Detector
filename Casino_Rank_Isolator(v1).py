...
for Name in ['Ace','Two','Three','Four','Five','Six','Seven','Eight',
    ...
    while(True):
        img = ImageGrab.grab(bbox=(123,316,346,612)) 
        img_np = np.array(img)
    ...
    if i <= 13: # Isolate rank
        rank = corner_thresh[20:128, 0:128] # Grabs portion of image that shows rank
        ...
        cv2.imshow(filename, rank)

    if i > 13: # Isolate suit
        suit = corner_thresh[70:336, 0:256] # Grabs portion of image that shows suit
